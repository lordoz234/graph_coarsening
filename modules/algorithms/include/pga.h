        ...
        while (deg(v)) {
            ...
            for (int i = graph.offset[v]; i < graph.offset[v + 1]; i++) {
                if (graph.weights[i] > weight &&
                    used.find(std::make_pair(v, graph.edges[i]))
                        == used.end() &&
                    !used1[graph.edges[i]]) {
                    e = graph.edges[i];
...
Matching PGA_with_max_weight_matching(const CSR<WeightType> &graph) {
    Matching result;
    int size = graph.n;
    std::vector <int> ed(size);
    std::vector <int> ed1(size);
    ...
    while (is_edge() != 0) {
        ...
        while (deg(v)) {
            ...
            if (e == -1) break;
            P.edge_e.push_back(e);
            P.edge_b.push_back(v);
            P.weights.push_back(weight);
            v = e;
        }
        Matching result_new = max_weight_matching(P);
        for (int i = 0; i < result_new.n; i++) {
            used1[result_new.edge_b[i]] = true;
            used1[result_new.edge_e[i]] = true;
            result.edge_b.push_back(result_new.edge_b[i]);
